...
In this exericse, you will explore the sample todolist application built with ionic 4 framework and perform following tasks:

* Generate client SDK based on the openAPI specification. This SDK will provide code to call TodoList REST APIs.
* Connect todolist app with the mock server created in  exercise-2.
* Extend code to update an item in the todolist app.
...
Generate client sdk based on specification created in [exercise-1](exercise-1.md). For reference, this repo contains a complete version `todolist.yaml` in folder. 
...
```
> Explore the sdk generated by openapi-generator tool in folder src/app/rest .
...
## Step 7 - Extend app to allow user to update an item
  ...
  * Include function update item by id in `src/app/services/item.service.ts`  :
  ...
  ```
   ...
   > Note: To have a better understanding, review code in `update-item.page.ts` to see how `itemService` is being called

## Step 8 - Finally run app
```
ionic serve
```
And you should able to update an item by connecting to mock server. Verify by watching the network tab in  chrome dev tools
